...
exports.getStartPage = async (req, res) => {
  ...
  if (req.body.day) last += Number(req.body.day);
  ...
  if (id) {
    selectedTable = await tablesRepo
      .getTable(id)
      .then(table => {
        return table;
      })
  ...
  tablesRepo
    ...
    .then(async tables => {
      ...
      if (id) {
        ...
        context.date = dateFormat.getDateAfterDay(
  ...
  usersRepo
    ...
    .then(users => {
      res.render("admin.hbs", {
        ...
        isAdmin: req.session.isAdmin,
        open: config[0].open,
        close: config[0].close,
        maxDayAhead: config[0].maxDayAhead,
        id: config[0]._id
      });
    })
    .catch(() => {
